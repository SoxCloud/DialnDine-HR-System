...
const CREDIT_COLS = "A1:I";
const CREDIT_LOG_COLS = "A1:I";
...
export async function loadCredits(employees) {
  const rows = await getOptionalSheetData(SHEETS.creditLog, CREDIT_LOG_COLS);
  const totals = new Map(); // employeeId -> { amount, hours, updatedAt }
  for (const row of rows) {
    const employeeId = clean(row[COLS.creditLog.employeeId]);
    if (!employeeId) continue;
    const type = clean(row[COLS.creditLog.type]).toLowerCase();
    const amount = toPositiveNumber(row[COLS.creditLog.amount]);
    const hours = toPositiveNumber(row[COLS.creditLog.hours]);
    const updatedAt = clean(row[COLS.creditLog.createdAt]);
    if (!totals.has(employeeId)) {
      totals.set(employeeId, { amount: 0, hours: 0, updatedAt: "" });
    }
    const entry = totals.get(employeeId);
    if (type === "wrong_order" || type === "wrong-order" || type === "wrong order") {
      entry.amount += amount;
    } else if (type === "absconded_hours" || type === "absconded-hours" || type === "abscond") {
      entry.hours += hours;
    } else {
      entry.amount += amount;
      entry.hours += hours;
  ...
  return employees.map((employee) => {
    const entry = totals.get(employee.employeeId) || { amount: 0, hours: 0, updatedAt: "" };
    return {
      employeeId: employee.employeeId,
      name: employee.name,
      amount: entry.amount,
      hours: entry.hours,
      credits: entry.amount + entry.hours,
...
/**
 * All credit log entries, newest first. Resolves employee names.
 * Each entry: { date, employeeId, name, type, store, customerDetails, reason, amount, hours, createdAt }.
 */
export async function loadCreditLog(employees) {
  const roster = new Map(employees.map((employee) => [employee.employeeId, employee.name]));
  const rows = await getOptionalSheetData(SHEETS.creditLog, CREDIT_LOG_COLS);
  return rows
    .filter((row) => clean(row[COLS.creditLog.employeeId]))
    .map((row) => {
      const employeeId = clean(row[COLS.creditLog.employeeId]);
      return {
        date: clean(row[COLS.creditLog.date]),
        employeeId,
        name: roster.get(employeeId) || employeeId,
        type: clean(row[COLS.creditLog.type]) || "wrong_order",
        store: clean(row[COLS.creditLog.store]),
        customerDetails: clean(row[COLS.creditLog.customerDetails]),
        reason: clean(row[COLS.creditLog.reason]),
        amount: toPositiveNumber(row[COLS.creditLog.amount]),
        hours: toPositiveNumber(row[COLS.creditLog.hours]),
        createdAt: clean(row[COLS.creditLog.createdAt]),
      };
    })
...
/**
 * Month-end candidates: for `month` (YYYY-MM), employees who worked any hours
 * after the 25th. Returns [{ employeeId, name, hoursAfter25th }] sorted desc.
 * These are the people paid up to the end of the month whose post-25th hours
 * must be credited the following month.
 */
export async function loadMonthEndCandidates(month, employees, scheduleByDate = new Map()) {
  const roster = new Map(employees.map((employee) => [employee.employeeId, employee.name]));
  const monthStart = `${month}-26`;
  const nextMonth = new Date(`${month}-01`);
  nextMonth.setUTCMonth(nextMonth.getUTCMonth() + 1);
  const monthEnd = `${nextMonth.getUTCFullYear()}-${String(nextMonth.getUTCMonth() + 1).padStart(2, "0")}-31`;
  ...
  for (const row of rows) {
    const date = clean(row[COLS.attendance.date]);
    const employeeId = clean(row[COLS.attendance.employeeId]);
    if (!date || !employeeId || date < monthStart || date > monthEnd) continue;
    totals.set(employeeId, (totals.get(employeeId) || 0) + hoursFor(row, scheduleByDate, employeeId));
  }

  return [...totals.entries()]
    .map(([employeeId, hoursAfter25th]) => ({
      employeeId,
      name: roster.get(employeeId) || employeeId,
      hoursAfter25th: Math.round(hoursAfter25th * 100) / 100,
    }))
    .filter((entry) => entry.hoursAfter25th > 0)
    .sort((a, b) => b.hoursAfter25th - a.hoursAfter25th);
}
